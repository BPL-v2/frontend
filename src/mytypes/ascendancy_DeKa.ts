...
// "Undecided" is a team-sheet-only placeholder (no real ascendancy, no
// thumbnail/image) - deliberately NOT a key in `ascendancies` below, since
// that object's keys are also iterated elsewhere (e.g.
// SubmissionFormModal's "Ascendancies used" picker) where "Undecided" isn't
// a valid choice. Exported separately so the sheet form and its chart can
// still give it a distinct, attention-grabbing color.
